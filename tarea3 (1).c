...
int main(int argc, char* argv[]) {
  ...
  pthread_t cliente1,cliente2,caja1,caja2;
  datos_compartidos_t datos_compartidos;

  srandom(time(NULL));

  if (argc == 5) {
    datos_compartidos.capacidad_caja=random_entre(5,15);
    datos_compartidos.cantidad_clientes= random_entre(1,10);
 ...
 error = pthread_create(&cliente2, NULL, produce, &datos_compartidos);
  if (error == 0) {
    error = pthread_create(&caja2, NULL, consume, &datos_compartidos);
